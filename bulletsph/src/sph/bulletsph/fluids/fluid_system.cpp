...
void FluidSystem::Initialize ( int mode, int particalNumMax )
{
	if ( mode != BFLUID ) {
		printf ( "ERROR: FluidSystem not initialized as BFLUID.\n");
	}
	PointSet::Initialize ( mode, particalNumMax );
	
	FreeBuffers ();
	AddBuffer ( BFLUID, sizeof ( Fluid ), particalNumMax );
	...
	SPH_Setup ();
	Reset ( particalNumMax );	
}
void FluidSystem::uninitialize()
{
	PointSet::uninitialize ();
}
...
void FluidSystem::SPH_setInitVolumeBox(
	const double minx, const double miny, const double minz,
	const double maxx, const double maxy, const double maxz)
{
		m_Vec [ SPH_INITMIN ].Set ( minx, miny, minz );
		m_Vec [ SPH_INITMAX ].Set ( maxx, maxy, maxz );
}
void FluidSystem::SPH_setObstacleBox(
	const double minx, const double miny, const double minz,
	const double maxx, const double maxy, const double maxz)
{
		m_Vec [ SPH_VOLMIN ].Set ( minx, miny, minz );
		m_Vec [ SPH_VOLMAX ].Set ( maxx, maxy, maxz );
}